...
<?php
    ...
    if ( !empty($_POST))
    {
        ...
        if ($valid) {
            ...
            $sql = "Update usuario set dni=?,razon=?,contacto=?,telefono=?,email=? where id=?";
            $stmt = $PDO->prepare($sql);
            $stmt->execute(array($nit,$name,$contact,$phone,$email,$id));
            $PDO = null;
            header("Location: index.php");
        }
    }
...
<div class="container">
    ...
    <div class="form-group <?php echo !empty($fnitError)?'has-error':'';?>">
             <label for="inputNit">NIT</label>
                <input type="text" class="form-control" required="required" id="inputNit" value="<?php echo !empty($nit)?$nit:'';?>" name="nit" placeholder="DNI">
       </div>
       <br />
           <div class="form-group <?php echo !empty($fnitError)?'has-error':'';?>">
            <label for="inputName">Nombre</label>
            <input type="text" class="form-control" required="required" id="inputName" value="<?php echo !empty($name)?$name:'';?>" name="name" placeholder="Razon Social">
    </div>
           <br />
           <div class="form-group <?php echo !empty($fcontact)?'has-error':'';?>">
            <label for="inputName">Contacto:</label>
            <input type="text" class="form-control" required="required" id="inputName" value="<?php echo !empty($contact)?$contact:'';?>" name="contact" placeholder="Nombre Contacto">
    </div>

             <br />
           <div class="form-group <?php echo !empty($fcontact)?'has-error':'';?>">
            <label for="inputName">Telefono:</label>
            <input type="text" class="form-control" required="required" id="inputPhone" value="<?php echo !empty($phone)?$phone:'';?>" name="phone" placeholder=" Número Telefono Contacto">
    </div>

             <br />
           <div class="form-group <?php echo !empty($fcontact)?'has-error':'';?>">
            <label for="inputName">Correo:</label>
            <input type="text" class="form-control" required="required" id="inputEmail" value="<?php echo !empty($email)?$email:'';?>" name="email" placeholder="Correo Electronico">
    </div>
