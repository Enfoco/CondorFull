...
<?php
    ...
    if ( !empty($_POST))
    {
        ...
        if ($valid) {
            ...
            $sql = "Update etapa set detalle=? where id=?";
            ...
            require '../../model/db.php';
        $PDO->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM etapa where id = ?";
        $stmt = $PDO->prepare($sql);
...
<div class="container">
                    ...
                    <div class="row">
                    <div class="row">
                        <h3>Actualziar Configuración Etapa</h3>
                    </div>
            
                    <form method="POST" action="update.php?id=<?php echo $id?>">
    <div class="form-group <?php echo !empty($fnameError)?'has-error':'';?>">
        <label for="inputFName">Detalle</label>
        <input type="text" class="form-control" required="required" id="inputName" value="<?php echo !empty($name)?$name:'';?>" name="name" placeholder="Cantidad de Etapa">
