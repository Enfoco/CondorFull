...
<?php
    ...
    if ( !empty($_POST))
    {
        ...
        $valid = true;
        if(empty($name)) {
            $nameError = 'Por Favor Ingrese Nombre del Empleado';
            $valid = false;
        }
        ...
        if ($valid) {
            ...
            $sql = "Update empleado set nombre=?, apellido=?, telefono=? where id=?";
            $stmt = $PDO->prepare($sql);
            $stmt->execute(array($name,$lastname,$phone,$id));
            $PDO = null;
            header("Location: index.php");
        }
    }
...
<div class="container">
    ...
    <div class="form-group <?php echo !empty($fnameError)?'has-error':'';?>">
        ...
        <input type="text" class="form-control" required="required" id="inputName" value="<?php echo !empty($name)?$name:'';?>" name="name" placeholder="Nombre">
        </div>
      <br />
         <div class="form-group <?php echo !empty($flastnameError)?'has-error':'';?>">
       <label for="inputFLastname">Apellido</label>
        <input type="text" class="form-control" required="required" id="inputLastname" value="<?php echo !empty($lastname)?$lastname:'';?>" name="lastname" placeholder="Apellidos">
  
       </div>
             <br />
            <div class="form-group <?php echo !empty($fphoneError)?'has-error':'';?>">
       <label for="inputFPhone">Telefono</label>
        <input type="text" class="form-control" required="required" id="inputPhone" value="<?php echo !empty($phone)?$phone:'';?>" name="phone" placeholder="Numero Telefonico">
